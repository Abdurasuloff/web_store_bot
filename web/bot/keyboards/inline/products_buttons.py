...
async def shop_keyboard(product_id, user_id, subcategory_id):
    ...
    markup.row(
        InlineKeyboardButton(
            text="⬅️ Ortga", callback_data=back_callback.new(category_id=0, subcategory_id=subcategory_id))
        )
...
async def buy_product(total_price, description):
    ...
    markup.insert(
        InlineKeyboardButton(
            text="⬅️ Ortga", callback_data=back_callback.new(category_id=0, subcategory_id=0))
        )
...
back_button.insert(
    InlineKeyboardButton(
        text="⬅️ Orqaga", callback_data=back_callback.new(category_id=0, subcategory_id=0)
    )
)